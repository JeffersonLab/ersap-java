...
public class InfluxDbReporter implements DpeReportHandler {
    ...
    private static final String DEFAULT_DB_HOST = "ersapweb.jlab.org";
    ...
    private final String dbName = "ersap";
    ...
    public void handleReport(DpeRegistrationData dpeRegistration, DpeRuntimeData dpeRuntime) {
        ...
        if (jinFxConnected) {
            ...
            System.out.printf("%s: reporting for DPE = '%s'  session = '%s'%n",
                      ErsapUtil.getCurrentTime(), dpeRegistration.name(), session);

            tags.put(ErsapConstants.SESSION, session);

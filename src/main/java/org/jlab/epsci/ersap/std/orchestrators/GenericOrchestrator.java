...
import org.jlab.epsci.ersap.std.orchestrators.CoreOrchestrator.DpeCallBack;
...
//        removeStageDirectories();
    ...
    protected void printStartup() {
        System.out.println("==========================================");
        System.out.println("            ERSAP Orchestrator            ");
        System.out.println("==========================================");
    ...
    private void tryLocalNode() {
        if (options.useFrontEnd) {
            ...
            Map<ErsapLang, DpeName> localDpes = orchestrator.getRegisteredDpes(2).stream()
                    .filter(this::isLocalDpe)
                    .collect(Collectors.toMap(DpeName::language, Function.identity()));

            Set<ErsapLang> appLangs = setup.application.getLanguages();
            Set<ErsapLang> dpeLangs = localDpes.keySet();

            if (dpeLangs.containsAll(appLangs)) {
                for (ErsapLang lang : appLangs) {
                    DpeName dpe = localDpes.get(lang);
                    dpeCallback.callback(new DpeInfo(dpe, cores, EnvUtils.ersapHome()));
                }
            }
        }
    }
    ...
    static class CommandLineBuilder {
        ...
        private DpeName parseFrontEnd() {
            String frontEnd = options.valueOf(this.frontEnd)
                                     .replaceFirst("localhost", ErsapUtil.localhost());
            try {
        ...
        public String usage() {
            String wrapper = "ersap-orchestrator";
            return String.format("usage: %s [options] <servicesFile> <datasetFile>", wrapper)
                ...
                + OptUtils.optionHelp(frontEnd, "frontEnd",
                        "The name of the ERSAP front-end DPE")

...
final class FarmCommands {
    ...
    private static String defaultConfigFile() {
        ...
        Path ymlPath = ERSAP_USER_DATA.resolve("config/services.yaml");
    ...
    private static String defaultFileList() {
        String out = "undefined";
        Path filesPath = ERSAP_USER_DATA.resolve("config/files.txt");
    ...
    private abstract static class FarmCommand extends AbstractCommand {
        ...
        protected Path getJobScript(String ext) {
            ...
            return ERSAP_USER_DATA.resolve("config/" + name + ext);
        }
    }
    ...
    static class RunFarm extends FarmCommand {
        ...
        RunFarm(Context context) {
            super(context, "farm", "Run ERSAP data processing on the farm.");
        }
...
//                                config.setValue(Config.MAX_THREADS, 64);
        ...
        private int splitIntoMultipleJobs(int filesPerJob) throws IOException, TemplateException {
            ...
            try {
                Path dotDir = Paths.get(ERSAP_USER_DATA.toString(), "config",
                    "." + runUtils.getSession());
        ...
        private String getClaraCommand() {
            ...
            Path wrapper = FileUtils.ersapPath("lib", "ersap", "run-ersap");
...
//            return cmd.toString();
        ...
        private String getClaraCommandAffinity(String affinity,
            ...
            String exec = FileUtils.ersapPathAffinity(
                affinity,
                File.separator + "lib",
                File.separator + "ersap",
                File.separator + "/run-ersap ");
...
//                            subFileList.toString())).append("> /dev/null 2>&1 &\n");
        ...
        private Model createDataModel() {
            ...
            if (monitor != null) {
                model.put("ersap", "monitorFE", monitor);
            }
    ...
    private static Path getTemplatesDir() {
        String devDir = System.getenv("ERSAP_TEMPLATES_DIR");
        if (devDir != null) {
            return Paths.get(devDir);
        }
        return FileUtils.ersapPath("lib", "ersap", "templates");
    }

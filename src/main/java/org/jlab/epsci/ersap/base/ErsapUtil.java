...
/**
 * Extra helper methods for ERSAP orchestrator and services.
 */
@ParametersAreNonnullByDefault
public final class ErsapUtil {
    ...
    public static final Pattern CANONICAL_NAME_PATTERN =
            Pattern.compile("^" + ErsapComponent.CANONICAL_NAME_REGEX + "$");
    ...
    /**
     * Checks if the given name is a proper ERSAP canonical name.
     * <p>
     * A canonical name should have any of the following structures:
     * <pre>
     * {@literal <host>%<port>_<language>}
     ...
     * @return true if the string is a ERSAP canonical name, false if not
     */
    ...
    /**
     * Gets the DPE name from the given ERSAP canonical name.
     *
     * @param canonicalName a ERSAP canonical name
     * @return the DPE name
     */
    ...
    /**
     * Gets the container name from the given ERSAP canonical name.
     * This returns just the container name part, without the DPE name
     * (i.e. not a container canonical name).
     *
     * @param canonicalName a ERSAP canonical name with a container part
    ...
    /**
     * Gets the container canonical name from the given ERSAP canonical name.
    ...
    /**
     * Gets the service engine name from the given ERSAP canonical name.
    ...
    /**
     * Gets the DPE host address from the given ERSAP canonical name.
     *
     * @param canonicalName a ERSAP canonical name
    ...
    public static String getDpeHost(String canonicalName) {
        ...
        int portSep = canonicalName.indexOf(ErsapConstants.PORT_SEP);
        if (portSep > 0) {
            return canonicalName.substring(0, portSep);
        } else {
            int langSep = canonicalName.indexOf(ErsapConstants.LANG_SEP);
            return canonicalName.substring(0, langSep);
        }
    }

    /**
     * Gets the DPE port from the given ERSAP canonical name.
     *
     * @param canonicalName a ERSAP canonical name
    ...
    /**
     * Gets the DPE language from the given ERSAP canonical name.
     *
     * @param canonicalName a ERSAP canonical name
    ...
    public static String getDpeLang(String canonicalName) {
        ...
        return dpeName.substring(dpeName.indexOf(ErsapConstants.LANG_SEP) + 1);
    }

    /**
     * Gets the default DPE port for the given language.
     *
     * @param lang the given ERSAP language
     * @return the default port for a DPE of the language
     */
    public static int getDefaultPort(ErsapLang lang) {
     ...
     * Gets the default DPE port for the given language.
     *
     * @param lang a supported ERSAP language
    ...
    private static int getPort(String fullName, int index) {
        switch (fullName.charAt(index)) {
            case 'j': case 'J':
                return ErsapConstants.JAVA_PORT;
            case 'c': case 'C':
                return ErsapConstants.CPP_PORT;
            case 'p': case 'P':
                return ErsapConstants.PYTHON_PORT;
            default:
